...
One can run pytest from a root directory, and then pytest will run all of the UnitTesterSG tests automatically to make sure they still pass (relative to stored results).  The underscore in the file names are partially to be compatible with pytest. pytestDriver.py will run all the UnitTesterSG tests in each subdirectory.  So essentially one can have a UnitTests directory with subdirectories containing unit tests for different functions.  Running pytestDriver.py will test all of the test files in each directory.
